...
async fn request<T: serde::de::DeserializeOwned>(path: &str, post: bool) -> Result<T, String> {
    ...
    let response = request.send().await.map_err(|_| "Cannot reach EPIC. If a creation request was interrupted, check review history before creating another review.".to_string())?;
    if !response.ok() {
        return Err(response
            .json::<ReviewApiError>()
            .await
...
async fn create_review() -> Result<SavedReview, String> {
    #[cfg(all(feature = "hydrate", not(feature = "ssr")))]
    {
        request("/api/reviews", true).await
    }
...
fn ReviewWorkbench(data: ReviewPageData) -> impl IntoView {
                    ...
                    history.update(|entries| {
                        entries.insert(
                            ...
                            ReviewHistoryEntry {
                                ...
                                summary: saved.document.summary.clone(),
                            },
                        )
                    });
                    selected.set(Some(ReviewDetail {
                        review: saved,
                        comparison: None,
                        comparison_error: None,
                    }));
                    message.set(Some(format!("Review #{id} saved.")));
                    match open_review(id).await {
                        Ok(detail) => selected.set(Some(detail)),
                        Err(e) => error.set(Some(format!(
                            "Review was saved, but comparison could not be loaded: {e}"
                        ))),
                    }
                }
    ...
    view! {
        ...
        <button type="button" id="create-review" on:click=create disabled=move || busy.get()>
            {move || if busy.get() { "Working…" } else { "Create portfolio review" }}
        </button>
